  ...
  return (
    ...
    >
      <div className="mx-auto grid max-w-5xl gap-6 lg:grid-cols-[0.85fr_1.15fr]">
        <div className="rounded-[32px] border border-white/15 bg-white/10 p-8 shadow-[0_0_80px_rgba(56,189,248,0.08)] backdrop-blur-xl">
        ...
        <motion.form initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="rounded-[32px] border border-white/15 bg-slate-950/60 p-6 backdrop-blur sm:p-8" onSubmit={(event) => event.preventDefault()}>
          ...
          <motion.button type="submit" whileHover={{ y: -2, scale: 1.02 }} whileTap={{ scale: 0.98 }} className="mt-6 rounded-full border border-cyan-400/40 bg-cyan-400/10 px-6 py-3 text-sm font-semibold text-cyan-200 shadow-[0_0_24px_rgba(34,211,238,0.12)] transition hover:bg-cyan-400/20">Send message</motion.button>
        </motion.form>
      </div>
    </SectionShell>
  );
